...
function extractChallengeData(content:string) {
  const challengeForm = extract(content, CHALLENGE_FORM_REGEXP, "could'nt find challenge form")!;
...
export async function handleCaptcha(content:string, request:RequestAPI<Request, CoreOptions, RequiredUriUrl>, options:any) {
